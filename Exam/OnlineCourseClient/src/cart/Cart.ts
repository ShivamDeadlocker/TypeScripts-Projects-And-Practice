...
//Loading the cart and calling the display cart function
export function loadCart(): void {
  ...
  $.ajax({
    ...
    success: function (data: CartDisplay[]) {
      displayCart(data);
    },
...
//Displaying the cart product data
export function displayCart(data: CartDisplay[]): void {
  let container = $("#cart-items");
  let total = 0;

  container.html("");

  data.forEach((item: CartDisplay) => {
...
//Proceeding to Payment also validating the payment info
